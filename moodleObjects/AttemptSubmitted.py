...
class AttemptSubmitted(moodleObjects.ModuleViewed):
	VERB = ["submitted", "http://activitystrea.ms/schema/1.0/submit"]
	...
	def __init__(self, objecttable, objectid, contextInstanceID):
		...
		if( row is None ):
			...
			if 'sumgrades' in row and row['sumgrades'] is not None: self.data['sumgrades'] = int(row['sumgrades'])
			self.data['questions'] = {}
